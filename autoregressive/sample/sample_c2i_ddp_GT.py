...
def main(args):
    ...
    # reference sample classes
    c_indices_ref = np.array(
                    [541,  97, 191, 172, 553, 604, 103, 733,
    ...
    for _ in pbar:
        ...
        for step, (data, target) in enumerate(loader):
            if len(c_indices_ref) == 0:  # Just one batch for GT
                break
            data = data.to(device, non_blocking=True)
            target = target.to(device, non_blocking=True)
            z_indices = data.reshape(data.shape[0], -1)
            c_indices = target.reshape(-1)
            print(c_indices.item())
            class_idx = c_indices.item()
            if class_idx not in c_indices_ref:
                continue
            else:
                maskk = c_indices_ref != class_idx
                c_indices_ref = c_indices_ref[maskk]
            decoding_step = len(decoding_mask)
            qzshape = [len(c_indices)*decoding_step, args.codebook_embed_dim, latent_size, latent_size]
            index_sample = z_indices.repeat(decoding_step,1)
            for step_id, mask in enumerate(decoding_mask):
                rows, cols = rows_cols[step_id]
                index_sample_step = index_sample[step_id]
                L = latent_size * latent_size
                valid_data = index_sample_step[mask]
                ...
                index_sample[step_id] = upsampled_tensor_1d
            samples = vq_model.decode_code(index_sample, qzshape) # output value is between [-1, 1]
            samples = torch.clamp(127.5 * samples + 128.0, 0, 255).permute(0, 2, 3, 1).to("cpu", dtype=torch.uint8).numpy()

            for i, sample in enumerate(samples):
                index = i * dist.get_world_size() + rank + total + 10 * (step + 1)
                Image.fromarray(sample).save(f"{sample_folder_dir}/{index:06d}_classid{class_idx}.png")
    ...
    if rank == 0:
        ...
        print(f"Total time for AR: {ARtime:.2f} seconds, batch size in each GPU: {args.per_proc_batch_size}, global batch size: {global_batch_size}, \
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-datapoints", type=int, default=None)
